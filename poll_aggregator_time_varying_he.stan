...
parameters {
  ...
  array[P, T, J] house_effects; // House effects for each pollster and party
}
...
model {
  ...
  for (p in 1:P) {
      house_effects[p,1] ~ normal(0, 0.05);      
    for (n in 2:T) {
      house_effects[p,n] ~ normal(house_effects[p,n-1], 0.05);      
    }
  }
  ...
  for (n in 1:N) {
    ...
    vector[J] log_vote_with_house_effect = log(vote[time]) + to_vector(house_effects[pollster_n,n,:]);
